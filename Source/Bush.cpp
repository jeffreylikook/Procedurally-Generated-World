...
Bush::Bush()
{
	...
	cout << "Bush Color " << height << endl;
	vec3 bushColor = generateColor();
...
Bush::Bush(vec3 mPosition, float mRotate, vec3 mScale) : position(mPosition), rotation_angle(mRotate), scale_factor(mScale)
{
...
//Move Location of Bush
void Bush::moveLocation(float x, float y, float z)
{
	position.x = x;
	position.y = y;
	position.z = z;
...
//Scale Bush
void Bush::scaleBush(float scale)
{
	scale_factor += scale;
}

vec3 Bush::generateColor() {
	return vec3(0.0, 0.31, 0.145);
}

//Draw Bush
void Bush::Draw(int shaderProgram, GLuint bushTextureID)
{
	/* Base */
	glBindVertexArray(cylinderVAO);

	mat4 partMatrix = translate(mat4(1.0f), vec3(0.0f, 0.0f, 0.0f)) * rotate(mat4(1.0f), 0.0f, vec3(0.0f, 1.0f, 0.0f)) * scale(mat4(1.0f), vec3(0.5f, 1.0f, 0.5f));
	...
	partMatrix = translate(mat4(1.0f), vec3(0.0f, 1.0f, 0.0f)) * rotate(mat4(1.0f), 0.0f, vec3(0.0f, 1.0f, 0.0f)) * scale(mat4(1.0f), vec3(1.0f, 1.5f, 2.0f));
...
void Bush::SetUniformMat4(GLuint shader_id, const char* uniform_name, mat4 uniform_value)
{
	glUseProgram(shader_id);
	glUniformMatrix4fv(glGetUniformLocation(shader_id, uniform_name), 1, GL_FALSE, &uniform_value[0][0]);
}

void Bush::SetUniformVec3(GLuint shader_id, const char* uniform_name, vec3 uniform_value)
{
	glUseProgram(shader_id);
	glUniform3fv(glGetUniformLocation(shader_id, uniform_name), 1, value_ptr(uniform_value));
}

GLuint Bush::setupModelEBO(string path, int& vertexCount)
{
...
bool Bush::loadOBJ2(
	...
	file = fopen(path, "r");
	if (!file) {
		printf("Impossible to open the file ! Are you in the right path Bush?\n");
		getchar();
		return false;
	}
